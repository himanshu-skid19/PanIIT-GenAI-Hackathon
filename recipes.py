...
recipe_dataset = pd.read_csv("Final_Recipes_scaled.csv")
...
def recipes_from_dataset(target_weight, number_of_days, age, weight, height, gender, pa, is_veg, meal_type = None, diet_type = "Balanced"):
    if meal_type is None:
        ...
        counter = 0
        breakfast_set = recipe_dataset[(recipe_dataset['Meal Type'] == "Breakfast") & (recipe_dataset['Diet Type'] == diet_type) & (recipe_dataset['Veg'] == is_veg)]
        lunch_set = recipe_dataset[(recipe_dataset['Meal Type'] == "Lunch") & (recipe_dataset['Diet Type'] == diet_type) & (recipe_dataset['Veg'] == is_veg)]
        dinner_set = recipe_dataset[(recipe_dataset['Meal Type'] == "Dinner") & (recipe_dataset['Diet Type'] == diet_type) & (recipe_dataset['Veg'] == is_veg)]
        while(counter < 20):
            counter += 1
